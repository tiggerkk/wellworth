...
- **Library** (`/quotes/library`): real-time search across quote text, author, title, and tags; faceted filters (**Categories**, multi-select **Tags**, **Favourites** toggle, **Source type**, **Language** — the Category/Source options come from the owner's configured lists); a lazy-loaded list of quote snippets; tap a row to edit, swipe-left to delete (hard, with confirm). When opened from a Show/Book detail it is constrained to that record's quotes.
