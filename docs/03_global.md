...
- The **Login** and **Onboarding** screens both show the WellWorth brand mark (a chop-seal "W"
  figure) via the shared `BrandMark` component (inline SVG, tinted `accent`) — one source for both,
  so they can't drift.
- The seal **geometry** is a single source in `src/lib/brand-mark.js` (plain JS so the raw-Node icon
  script can import it; a sibling `brand-mark.d.ts` types it). Both `BrandMark` and the icon
  generator read it, so the in-app logo and the home-screen icons stay in lock-step — edit the
  geometry once.
- The installed-app/home-screen icon and favicon are necessarily separate rasters
  (`public/*.png`/`.ico`, generated by `scripts/gen-icons.mjs` from that shared geometry), flattened
...
and no way past it except finishing or signing out. Its header shows the WellWorth brand mark
(the shared `BrandMark` component — same as the Login screen).
...
- **DISPLAY → Visible Modules** (secondary text "(Home)"): opens a full sheet listing every module in a
  **2-up combined grid** (`ReorderGrid`, same as the hub's 2-column layout) — **drag the grip to
  ...
  visible** (the last toggle refuses to turn off, following `ConfigListEditor`). Hiding only removes the
...
- `travel_importer_enabled` BOOLEAN NOT NULL DEFAULT true — single toggle surfacing **both** Travel
  importers (JSON Trips + CSV Expenses); on by default (added by `supabase/migrations/15_travel_profile_settings.sql`)
