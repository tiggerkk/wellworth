...
- A floating **+** (`ListFab`) opens **New Book**, shown only once the filtered list has at least one row (the bottom-nav **New Book** tab and the empty-state chip remain the other entry points).
