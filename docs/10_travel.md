...
- A floating **+** (`ListFab`) opens **New Trip**, shown only once the filtered list has at least one row (the bottom-nav **New Trip** tab and the empty-state chip remain the other entry points).
...
- **Import → Enable Bulk Trips Import / Export** (`profile.travel_importer_enabled`, **on by default**): a single toggle that surfaces the Import/Export launchers below. When off, a secondary note ("Turn this on to bulk-seed your trips from a JSON / CSV.") replaces them.
- **Import → Import JSON Trips** (listed first, paired with **Export JSON Trips**): a JSON array of trips → one combined review (per-trip day/stop counts + a pooled **new-cities** list with optional per-city geocode) → import as drafts.
- **Export JSON Trips**: `travel-export.ts` (pure) + `data/travel.listTripsForExport`. Every trip's full itinerary in **3 bulk queries total** (all trips, then every trip's days in one `.in(trip_id)`, then every one of those days' stops in one `.in(trip_day_id)`) — not one `getTripBundle` call per trip. Produces the same JSON shape `templates/travel.schema.json` describes, so the file re-imports unchanged.
  - Sorted by the date of each trip's **first stop, descending** — the date of the earliest day (by its own sort order) that has ≥1 stop; a trip with no dated first stop (no stops at all, or its first-stop day has no date set) sorts last regardless.
  - **Known gap:** a day's free-text **label** (set in Edit Trip) has no slot in the import schema and isn't restored by re-import — the importer only ever reads a day's `date` and `stops`.
- **Import → Import CSV Expenses** (listed second, **Import only — no Export**): a wide sheet (Trip, Date, category columns…, Cost, Re-imbursed) → detected-columns + **unknown-header mapping** + per-trip preview + **replace-per-trip** → import. Columns + rules: `templates/travel-expenses-import-guide.md`.
  - No Export CSV Expenses: the wide per-category format has no room for more than one amount per trip/day/category combination, so 2+ expenses sharing a day and category would have to collapse into one summed cell on export — losing the individual descriptions/count, keeping only the total. Deferred rather than shipped lossy; if you need to restore Expenses after a reset, re-run whatever CSV you originally imported.
