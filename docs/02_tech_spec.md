...
| Screen    | Entry   | Top-Left Icon | Post-Save Action | Post-Cancel Action | Routed |
| --------- | ------- | ------------- | ---------------- | ------------------ | ------ |
| View Item | Listing | <             | N/A (read-only)  | Return to Listing  | Yes    |

**Category 4 — special** (Wellness — no Dashboard, but has a Food / Activity picker)
...
- **A bottom-nav tab lights up via `NavLink`'s default prefix match, unless a `NavItem.isActive` override says otherwise.** `NavLink` without `end` matches any path that _starts with_ its `to`; add `end: true` when a sibling tab's route would otherwise nest under it (e.g. Quotes' `library` tab vs. its own `entry`/`settings`/`journal` siblings all sharing the `/quotes` prefix).
- **`isActive` override — when a listing screen's Edit/Detail/New route isn't path-nested under the listing's own prefix.** Insurance Policies happens to work with plain `NavLink` matching because its edit route (`/networth/insurance/:id`) nests under the tab's own path. Everywhere else — Shows, Books, Travel, Medical, Literature, Journal, Quotes — the Edit/Detail/New screens live at a sibling path (e.g. `/shows/:id`, not under `/shows/library`), so the listing tab needs an explicit `isActive: (pathname) => boolean` in `src/constants/modules.ts` that matches both the listing prefix and the sibling edit/detail pattern; `BottomNav` calls this instead of relying on `NavLink`'s own matching when it's present.
- **Regex gotcha: a module's catch-all edit pattern (`/^\/shows\/[^/]+$/` etc.) also matches that module's own `entry`/`settings`/other single-segment sibling routes** — exclude them explicitly (`pathname !== routes.shows.entry`) or the sibling tab lights up alongside the correct one (this bit Shows/Books' "New X" tab lighting up together with "Library" until fixed). Conversely, when "New X" should count as the listing tab too (Quotes' New Quote, Journal's New Journal Entry), include the entry path in the match instead of excluding it — check each module's own product rule rather than assuming one pattern fits all.
...
| Hook                     | Purpose                                                                                                                              |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ |
| `useAsync`               | Generic async-fetch state (`data`/`loading`/`error`/`refetch`), stale-data-preserving, optional shared cache. See gotchas F4/F8/F13. |
| `useProfile`             | Loads the signed-in user's `profile` row, seeded from a local cache for instant paint. See F21.                                      |
| `useProfileEditor`       | Fresh (non-cache-seeded) profile row for editors that copy it into local state on mount.                                             |
| `useEnsureProfile`       | First-login hook: creates the `profile` row + seeds starter data, idempotent.                                                        |
...
| `useSheetNavigate`       | Opens a routed sheet via React Router's background-location pattern.                                                                 |
| `useEscapeKey`           | LIFO stack of Esc handlers so the innermost overlay closes first.                                                                    |
| `useSessionState`        | Persists filter/sort criteria for a session (per-module `DEFAULT_*_CRITERIA` reset target).                                          |
| `useFontSizeSync`        | Reconciles the `data-font-scale` attribute from `profile.font_size` once it loads.                                                   |
| `useLiquidOnly`          | Net Worth — Dashboard/Monthly Entry "Liquid Only" view-filter state.                                                                 |
| `useMedicalTrends`       | Medical Dashboard's three bounded queries in one hook (latest-per-test, tracked-test history, recent reports).                       |
| `useNutrientReference`   | Session-scoped cache over the static nutrient reference table (see Wellness performance notes).                                      |
| `useLiteratureFavorites` | Loads + toggles a user's `poem_favorite` rows against the static corpus.                                                             |
| `useSpeech`              | Wraps the Web Speech API for Literature's read-aloud (voice pick, seek, auto-loop).                                                  |
| `useBarcodeScanner`      | Wraps ZXing's `BrowserMultiFormatReader` for the Wellness Diary Food Picker's barcode scan.                                          |
| `useReturnAfterLog`      | Wellness — routes back to the correct screen after logging a Diary entry (Diary vs. a deep link).                                    |
...
- **F16b — prefer optimistic local state over a global refresh-version bump; but bump on success too when another screen shares the read.** Prefer optimistic local state over bumping a module's global refresh version (`bumpTravel()` etc.) on every mutation: a version bump forces every subscriber to refetch its whole bundle (all days + all stops for one edit).
  - Keep the edited collection in local state, mutate it instantly, persist in the background, and bump on a write error (which refetches + re-seeds from server truth) so that screen self-corrects.
  - **Also bump on success for any field another screen reads** (a Dashboard/Zen tile, another module's filter dropdown, a shared cache key). Bumping only on error was the original rule here and shipped across Travel/Quotes/Shows/Books/Medical (`TravelEntry`'s stop/day handlers, `QuotesLibrary`/`ShowsLibrary`/`BooksLibrary`'s `remove`/`toggleFavorite`, `MedicalReports.remove`) — each left a sibling screen (Dashboard, Zen, Trips-filter dropdown) silently stale after a successful edit, since "other screens update on their next mount" is only true if something eventually re-bumps the version; a session that stays on the sibling screen never does. The fix is symmetric: bump in both the success and the catch branch, same shape as `JournalLibrary.remove()`, which had it right from the start.
  - A handler with genuinely no cross-screen reader (e.g. Travel's `setStopCompletion`, or an Entry form's own local-only drafts) can still skip the success bump — the risk is specifically an optimistic local-state screen that shares a cache key or another screen's independent subscription with a Dashboard/summary view.
  ...
  - Travel's Edit-Trip itinerary + expenses use this pattern — see `docs/10_travel.md` for the screen-level behavior this enables (per-day expense modal and the Expenses tab sharing one lifted state). `EditTrip`'s bundle-reseed effect already tolerated more frequent bumps by design (see its own comments), which is why extending the bump to itinerary success paths was a safe, additive fix rather than a rework.
  - **Also verify every reader actually subscribes.** The inverse gap — a Dashboard/summary screen that never calls `useXVersion()` at all — is a separate failure mode from the above (no amount of bumping helps if nothing is listening). `WellnessDashboard` had this: it fetched via a plain `useCallback` with no `version` in its deps, so Diary writes never triggered a refetch regardless of `bumpDiary()`'s timing. Fixed by adding the missing `useDiaryVersion()` subscription, matching `WellnessDiary`/`WellnessLibrary`.
...
## Shared CSV/JSON export helpers

`src/lib/csv.ts`, `src/lib/file-export.ts`, `src/components/ImportExportRow.tsx` — the counterpart to each module's bulk **importer**, letting the owner pull a module's data back out in the same file shape the importer accepts, so a full round-trip (export → truncate a module's tables → re-import) restores it.

- **`toCsv(rows)`** (`src/lib/csv.ts`) — the inverse of the existing `parseCsv`: serializes rows of string cells to RFC-4180 CSV text, quoting a cell only when it contains a comma, double-quote, or newline. Pure, no I/O.
- **`downloadCsv(filename, rows)` / `downloadJson(filename, data)`** (`src/lib/file-export.ts`) — the one place that turns a built CSV/JSON payload into an actual saved file: builds a `Blob`, clicks a throwaway `<a download>` anchor, revokes the object URL. Desktop-only mechanism by design (no Web Share API / native file picker) — the owner's stated primary use case is exporting from a laptop, not the installed iPhone PWA. `downloadCsv` prepends a UTF-8 BOM so Excel opens non-ASCII text correctly; `parseCsv` already strips a leading BOM, so re-importing works either way.
- **`ImportExportRow`** (`src/components/ImportExportRow.tsx`) — the shared "Import X" / "Export X" button pair rendered side by side in a module's Settings screen, replacing what used to be a lone Import button. Both buttons share the same `*_importer_enabled` toggle (relabeled "Enable X Import / Export" wherever an Export button was added) — there's no separate Export toggle.
- **Each module's own `*-export.ts`** (`journal-export.ts`, `quotes-export.ts`, `shows-export.ts`, `books-export.ts`, `wellness-food-export.ts`, `travel-export.ts`) is a pure row/JSON-builder — the inverse of that module's `*-import.ts` — taking already-fetched rows and producing the exact column/field spec the importer reads, plus a stable sort order for the printed file (independent of whatever order the underlying list query itself uses for on-screen display — see the Performance notes for each module below for the sort and any dedicated bulk query it needed).
- **Where a module's existing list query already selects every column the export needs** (Journal, Quotes, Shows, Books), the export screen calls that same `data/*.ts` list function directly — no new query, and no `select('*')` was added anywhere purely for export. Wellness Food and Travel Trips needed a dedicated bulk query (servings; the days+stops itinerary) — see their Performance notes entries below for why, and why each is still a small, fixed number of round trips rather than one per row.
- **Not every module has an Export**: Net Worth (Insurance, Net Worth) and Medical have Import only, and Travel's Expenses CSV has Import only — see each module's own doc for why.
...
- **F23 — typography scale + Dynamic Type.** Font sizes are **one rem-based scale** — the `@theme` `--text-*` tokens in `src/index.css` (`text-title/heading/field/body/label/caption/section`); see the role recipes in `docs/01_design_system.md`.
  ...
  - **Icons** ride the same lever via a `.tabler-icon` `transform: scale(...)` keyed off the same attribute — `transform`, not width/height, so the icon's layout box doesn't grow and add wrap pressure. Tabler icons carry this class from the library itself; the Wellness module's custom activity icon set (`src/constants/activity-icons.tsx`) applies the same class itself in its shared icon factory so it scales identically despite not being a Tabler icon.
...
- `useAsync` supports an opt-in shared cache (`{ key, version }`): a Dashboard and its matching Library/Trips/Zen screen share one cache entry per module. A hit at the exact version the module was last written skips the network call entirely, so switching between a module's Dashboard and Library no longer independently re-fetches the same full list. This only stays correct if every write that touches a field the _other_ screen reads bumps the version on success, not just on error — see **F16b** for the bug this produced when Library-side `remove`/`toggleFavorite` handlers bumped on error only, leaving the paired Dashboard/Zen screen's cached entry stale after a successful edit.
- **Export CSV Quotes/Shows/Books** each reuse that module's own `listQuotes`/`listShows`/`listBooks` as-is (called once, on demand, from that Settings screen) — each one's existing column selection is already a superset of its CSV's column spec, so no dedicated export query was added for any of the three. Each export's sort (Quotes: `created_at`/author/title; Shows: `type`/`status`/`start_date`; Books: `status`/`start_date`) runs once over the fetched rows inside that module's pure `*-export.ts` builder, independent of the Library's own display order.
- **Export JSON Trips** (`travel-export.ts` + `data/travel.listTripsForExport`) is the one export needing its own query: `getTripBundle` only loads one trip at a time (Edit Trip's shape), so exporting every trip runs **3 bulk queries total** — all trips, then every trip's days in one `.in(trip_id)`, then every one of those days' stops in one `.in(trip_day_id)` — grouped in memory into each trip's nested itinerary, rather than one `getTripBundle` call per trip (which would scale with trip count). The export's descending-by-first-stop-date sort computes each trip's sort key **once** per trip (a decorate/sort/undecorate pass) rather than inside the comparator itself, where a naive `.sort()` would recompute it on the order of `n log n` times instead of `n`.
- **Bulk "Refresh All from TMDB" (Shows Settings), dedicated query** (`listShowsForRefresh`, `src/data/show.ts`): `listShows`' column-trimmed select deliberately omits `tmdb_id`/`overview`/`original_language` (no list screen reads them), so the bulk refresh — which needs `tmdb_id` to even find its candidates — uses its own query rather than widening `SHOW_LIST_COLUMNS` for every Dashboard/Library read. It also filters server-side to `status in (want, watching)`, `type in (tv, documentary)`, and `tmdb_id is not null` — the same scope `bulkRefreshCandidates` re-checks client-side as a safety net — so Watched, Dropped, movie, and unmatched rows never leave the database instead of being fetched and discarded in memory. Reuses the existing `(user_id, status)` index; a dedicated composite index for the added `type`/`tmdb_id` filters wasn't justified at personal-library row counts.
- **Bulk "Refresh All from TMDB", bounded concurrency** (`refreshAllFromTmdb`, `src/lib/shows-bulk-refresh.ts`): a small worker pool (`REFRESH_CONCURRENCY = 4`) pulls from a shared index rather than awaiting each show's TMDB fetch + write sequentially — TMDB's public API comfortably tolerates a handful of concurrent requests (~50 req/s), so this cuts wall-clock time substantially for a library-wide refresh (e.g. ~100 shows) without needing the sliding-window rate limiter Books uses for Google's stricter per-key quota. Result order matches input order (workers write into a pre-sized array by index) regardless of which finishes first, so the Settings result list stays stable and easy to reason about.
...
- **Export CSV Journal** (`journal-export.ts`, called from Quotes Settings) reuses `listJournalEntries` as-is — its existing column selection (`day, journal_entry, mood, tags`) is already exactly the CSV's column spec, so no dedicated export query was added. The export sorts by `day` ascending for the printed file (oldest first, for readability) independently of the Library's own newest-first order — the sort runs once, in `buildJournalExportRows`, not inside the list query.
...
- **Memoized Food Picker results** (`WellnessDiaryFoodPickerSheet`): the local-food filter/map, USDA dedup/map, and the final score-sort (with a query) or alphabetical-sort (without one) are each behind `useMemo`, keyed on `userFoods`/`tab`/`usdaResults`/the trimmed query — same convention as apply\*View elsewhere. This screen re-renders on every keystroke (`query` state), so the chain would otherwise redo three filter/map/sort passes per keystroke even when the keystroke didn't change tab or user foods.
- **Diary group expand/collapse persisted, not recomputed per mount** (`WellnessDiary`): the per-group `expanded` map is held in `useSessionState` (`sessionStorage`), not local `useState` — so it's whatever the user last left it as, surviving a picker/detail sheet opening and closing over the Diary and navigating between days. No content-based auto-expand (a group with items doesn't open itself) — same convention as Net Worth Monthly Entry's Asset Type headers, which also just remembers the last toggle state rather than deriving it from data.
- **Export CSV Food, bulk servings query** (`listServingsForFoods`, `src/data/serving.ts`): the existing `listServings(foodId)` is one-food-at-a-time (Food Detail/Edit's shape); exporting every food's servings with that would be one query per food. `listServingsForFoods(foodIds)` instead fetches every serving for the whole export in **one** `.in(food_id, foodIds)` query, grouped into a `Map<food_id, Serving[]>` in memory — so the export's total query count (`listFoods` + `listServingsForFoods` + `getAllNutrients`) doesn't grow with the number of foods.
- **Export CSV Food, parallel independent fetches**: `listFoods()` and `getAllNutrients()` are fired together via `Promise.all` (neither depends on the other's result) rather than sequentially; only `listServingsForFoods` — which needs `listFoods`' resolved ids — waits on that first call. Same convention as the `fetchFoodRow`/`WellnessDiaryFoodDetailSheet` parallelization above, applied to a third call.
