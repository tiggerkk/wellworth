...
- A floating **+** (`ListFab`) opens **New Show**, shown only once the filtered list has at least one row (the bottom-nav **New Show** tab and the empty-state pill remain the other entry points).
