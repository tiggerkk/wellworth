...
- **Diary groups, in order:** Breakfast, Lunch, Dinner, Snacks, Supplements, Activities. Each header reads, left-to-right: **expand chevron · category icon · group name · kcal subtotal** (kcal sits right next to the name; activities show negative kcal coral) · ⟨spacer⟩ · **Delete · Copy · Paste · Add** icons (`IconTrash` / `IconCopy` / `IconClipboard` / `IconPlus`). **Delete** is a `ConfirmDeleteAction` (inline `Delete? ✓ ✗`, no browser dialog) — as is the day-level Delete in the top-right header. Category icons are colored via `WELLNESS_CATEGORY_COLOR` (see `docs/01_design_system.md` → Color tokens).
