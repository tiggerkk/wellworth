...
- **Highlighted Nutrients**: a grid of up to 8 chosen nutrients (a 4×2 grid when 8 are chosen), each a name, % of target, and a thin progress bar (via `NutrientBar`'s **`compact`** variant — name + % only, so the % isn't crowded out by the full nutrient name in the narrow 2-col grid). Chosen in Settings → Highlighted Nutrients (max 8). **Tapping a bar** opens the **Nutrient Sources** overlay (`WellnessNutrientFoodsOverlay`) for that nutrient, scoped to the viewed day — header shows the nutrient name + target + unit next to the close X; body lists every logged item (food **or** supplement — any `diary_entry` whose nutrient snapshot includes the tapped key) that contributed to it, each row showing the food label, its logged amount, and its contributed amount for that nutrient, sorted **descending by contributed amount**. Built entirely from the Diary's already-loaded day entries — no extra fetch. Not available on the Dashboard or Daily Report, since a multi-day range has no single per-entry contribution to attribute cleanly.
- **Diary groups, in order:** Breakfast, Lunch, Dinner, Snacks, Supplements, Activities. Each header reads, left-to-right: **expand chevron · category icon · group name · kcal subtotal** (kcal sits right next to the name; activities show negative kcal coral; **Supplements omits the kcal subtotal** — not a meaningful figure for that group) · ⟨spacer⟩ · **Delete · Copy · Paste · Add** icons (`IconTrash` / `IconCopy` / `IconClipboard` / `IconPlus`). **Delete** is a `ConfirmDeleteAction` (inline `Delete? ✓ ✗`, no browser dialog) — as is the day-level Delete in the top-right header. Category icons are colored via `WELLNESS_CATEGORY_COLOR` (see `docs/01_design_system.md` → Color tokens).
- **Expand/collapse persists for the session:** each group's expand/collapse state is remembered in `sessionStorage` (same mechanism as Net Worth Monthly Entry's Asset Type headers) — it's whatever the user last left it as, with no content-based auto-expand (a group with items doesn't automatically open). State survives navigating to another day and back, and a picker/detail sheet opening and closing over the Diary. A **group Paste** expands the target group so the pasted items are visible; a **day Paste** doesn't change any group's expand state. Expanded shows the logged entries; **tap an entry** to edit it (reopens Diary Food Detail / Diary Activity Detail with **RESET** + **SAVE**), **swipe-left** reveals Delete, and a **drag handle** reorders items within the group (the `ReorderList` component shared with Edit Trip; persisted via `diary_entry.sort_order`).
...
- Result rows are two lines: **line 1** is the food name (supplements with a pill icon) and a heart on the right; **line 2** shows the source tag (USDA / Custom / OFF) and `<n> nutrients · <serving>`. **With a search query**, results are ordered by name-match relevance, then nutrient count, then name — same ordering across Favorites/Custom/All. **With no query**, results fall back to alphabetical-by-name (also the same across all three tabs). Your saved foods have an interactive heart to toggle favorite; raw USDA results do not have a heart and are favorited from **Diary Food Detail**.
...
- **Exercises** list: each exercise expands to sets logged as reps × weight, with "Add set"; an "Add exercise" button builds the session. Sets persist to `strength_set` for progress tracking, ordered by `exercise_order` (the owner's chosen exercise order, stamped from the exercise's position in the draft list on save) then `set_number`. Reps/weight are editable drafts that can be **emptied** while typing; **Add set duplicates the previous set's reps + weight**. Each exercise card has **up/down chevrons** to reorder it within the session (disabled at the top/bottom bound) and a **delete** icon to remove it outright — both act on the local draft only, since the whole exercise list is written atomically on save (no per-move persistence call). Validation: for any **named** exercise every set needs **reps > 0** and **weight (kg) ≥ 0** (0 = bodyweight); an **unnamed** exercise is fine left blank (dropped on save) but is flagged if any reps/weight field has been filled in — so typed sets aren't silently lost.
...
- **Icon**: an icon picker (the keys of `ACTIVITY_ICONS`); optional, defaults to `running`.
...
- **Strength sets**: an activity entry's `exercises[].sets[]` become `strength_set` rows after the parent `diary_entry` insert returns its generated id (`buildStrengthSetInserts`, one bulk `createSets` call for every entry's sets across the whole import, not one insert per entry). Each exercise's `exercise_order` is stamped from its position in the `exercises` array, so re-importing preserves exercise order.
...
- A strength activity entry's `strength_set` rows (already ordered by `exercise_order` then `set_number`, from `listSetsForEntries`) are grouped into one `exercises` block per distinct exercise name, in first-seen order — which is therefore the owner's chosen exercise order (see Diary Activity Detail — strength type above).
...
- **Retry once on transient gateway failures:** USDA's API gateway is intermittently flaky — an identical, well-formed request can 404 one moment and succeed the next — so both the search and detail calls retry once after a short delay on a network error or a likely-transient status (404, 429, 5xx) before surfacing the failure. A persistent failure still fails the same way after the retry. The search overlay and Food Picker's All tab show the real thrown message (e.g. "USDA search failed (404)") rather than a generic missing-key hint, since the two failure modes need different fixes.
...
- `icon` TEXT NULL — activity icon key (e.g. `'karate'`), resolved at render time via `ACTIVITY_ICONS[icon]` in `src/constants/activity-icons.tsx` (re-exported from `src/constants/wellness.ts`). The activity library uses a custom hand-drawn stroke-based SVG icon set rather than Tabler icons, since Tabler doesn't cover activities like Push-Ups, Sit-Ups, or 太极, and some available Tabler icons don't read clearly at small sizes. Null/unknown falls back to `running`.
...
| ---------------------- | -------- | ---------------- | -------------- | ------------------------------------------- | ----------------------------------------- | ---------- |
| Body Combat            | duration | 60               | vigorous       | {"light":3.0,"moderate":5.9,"vigorous":7.0} | High-intensity martial-arts cardio        | bodycombat |
| 八段锦 (Baduanjin)     | duration | 10               | light          | {"light":3.0}                               | Gentle qigong                             | baduanjin  |
| 太极                   | duration | 10               | light          | {"light":2.3}                               | Taichi                                    | taichi     |
| Stretching             | duration | 10               | light          | {"light":2.3}                               | Shoulder/Neck stretches                   | stretching |
| Yoga                   | duration | 15               | light          | {"light":2.5,"moderate":3.5}                | General                                   | yoga       |
| Push-Ups               | duration | 2                | moderate       | {"light":3.8}                               | 10-20 reps at steady pace                 | pushup     |
| Sit-Ups                | duration | 2                | moderate       | {"light":3.8}                               | Standard sit-ups at steady pace           | situp      |
| Weights - General      | strength | 20               | moderate       | {"light":3.0,"moderate":3.5}                | 8–15 reps, standard rest                  | weights    |
| Weights - Powerlifting | strength | 20               | vigorous       | {"vigorous":6.0}                            | Heavy sets                                | weights    |
| Weights - Circuit      | strength | 20               | vigorous       | {"vigorous":8.0}                            | Fast-paced, minimal rest, high heart rate | weights    |
| Swimming               | duration | 30               | moderate       | {"light":3.0,"moderate":5.9,"vigorous":6.5} | Leisurely                                 | swimming   |
| Walking                | duration | 30               | moderate       | {"moderate":3.3}                            | ~3 mph                                    | walking    |
| Running - Jog          | duration | 30               | moderate       | {"moderate":5.9}                            | ~4 mph                                    | running    |
| Running - Fast         | duration | 30               | vigorous       | {"vigorous":9.8}                            | ~6 mph                                    | running    |
