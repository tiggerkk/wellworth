...
- **Exchange rates** panel: a **Collapsible**, collapsed by default, titled **Exchange rates** + `text-compact` grey **(as of 1st of the month from Frankfurter)**; expands to **4 rows of 2 currencies** — CNY/USD, AUD/CAD, JPY/KRW, SGD/THB — each auto-fetched (with ↻ refetch) and overridable. Native → HKD as of the 1st of the month, via Frankfurter (see `docs/02_tech_spec.md` → Shared external APIs). Expand/collapse state shares the same **`networth-entry-expanded`** session map as the asset-type sections below (key `fx`), so it persists across months the same way. For a saved month, rates hydrate from that month's frozen entries where a currency was actually used; any fetchable currency with no frozen entry that month (e.g. a month saved before a currency was added) falls back to a live Frankfurter fetch so the panel never shows unexplained blanks.
...
- `currency` TEXT — `'HKD' | 'CNY' | 'USD' | 'AUD' | 'CAD' | 'JPY' | 'KRW' | 'SGD' | 'THB'`
