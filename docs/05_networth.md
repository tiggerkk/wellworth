...
- Below it, the live **NET WORTH** total (HKD, updates live as you edit), the **Liquid Only** toggle, and **Import CSV** (manual importer) share one line. The header stays **pinned** while the asset-type list scrolls beneath; **while a month loads**, the header stays pinned (with a `—` total placeholder) and `Loading…` shows in the body below.
...
- **Each asset-type section has a colored left stripe + tinted header** (4px left stripe + `color-mix … 14%` header tint) keyed to `ASSET_TYPE_COLORS` (the same per-type palette as the Dashboard "By asset type" dots — defined in `src/lib/networth.ts`), and the **Import CSV** action is accent.
...
- **Schedule comparison**: `buildScheduleComparisonRows(a, b)` merges two schedule versions' points on the union of `age`, ascending, for the Compare Schedules overlay — each row carries `age`, `policy_year`, and each schedule's `cash_value` + `surrenderGainPctPerYear` (both `null` when that schedule has no point at that age). Native-currency formatters `formatNative` / `formatNativeCompact` sit alongside the existing HKD-only `formatHkd` / `formatHkdCompact`, used wherever a figure must stay in a policy's own currency rather than convert to HKD.
