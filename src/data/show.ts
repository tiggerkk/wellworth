...
/**
 * Idempotent bulk upsert for the CSV importer. Dedups on `type` + case-insensitive title against the
 * user's existing rows (and within the batch — last wins): an existing match is **updated**, a new
 * title is **inserted**. Re-running the same file therefore never duplicates.
 */
export async function saveImportedShows(
  ...
  for (const s of existing ?? []) idByKey.set(dedupKey(s.title, s.type), s.id)

  // Collapse in-file duplicates (same key) — last wins.
  const byKey = new Map<string, ImportShowRow>()
  for (const p of payloads) byKey.set(dedupKey(p.title, p.type), p)
