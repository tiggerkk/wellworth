...
import { BOOK_ENTRY_FIELDS } from '../lib/books'
import type { Tables, TablesUpdate } from '../types/database'

/**
 * Pick which Entry/Edit fields are visible. Mirrors the Shows Visible Fields sheet: auto-saves each
 * toggle. `profile.book_visible_fields` is NULL until the owner customizes (all fields visible); the
 * first toggle writes the explicit array.
 */
...
function Picker({
  ...
  const allKeys = BOOK_ENTRY_FIELDS.map((f) => f.key)
  const [visible, setVisible] = useState<string[]>(profile.book_visible_fields ?? allKeys)

  function toggle(key: string, on: boolean) {
    const next = on ? [...visible, key] : visible.filter((k) => k !== key)
    setVisible(next)
    void save({ book_visible_fields: next })
  }

  return (
    <div className="flex-1 overflow-y-auto p-4">
      <p className="mb-2 px-1 text-xs text-text-secondary">
        Choose which fields appear on the Add/Edit Book form. Title and Status (and
        Search) are always shown.
      </p>
      <div className="overflow-hidden rounded-card border border-border bg-surface">
        {BOOK_ENTRY_FIELDS.map((f) => (
          <div
            key={f.key}
