...
export function BottomNav({ module }: { module: ModuleDef }) {
  ...
  return (
    <nav className="sticky bottom-0 z-10 flex border-t border-border bg-surface pb-[env(safe-area-inset-bottom)]">
      ...
      <NavLink to={routes.home} className={({ isActive }) => tabClass(isActive)}>
      ...
      {module.tabs.map(({ to, label, Icon, end, isActive: isActiveOverride }: NavItem) =>
        isActiveOverride ? (
          <NavLink
            key={to}
            to={to}
            className={tabClass(isActiveOverride(location.pathname))}
          >
