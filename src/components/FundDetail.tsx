...
export function FundDetail({ data }: { data: FundDetailData }) {
  ...
  const rows: { label: string; value: string }[] = [
    { label: 'Total Value (HKD)', value: formatHkd(data.valueHkd) },
    {
    ...
    { label: 'Priced as of', value: str(d.nav_as_of) || '—' },
    { label: 'Total Cost', value: money(d.total_cost, 'HKD') },
    {
      label: 'Return Rate',
      value: n(d.return_rate) == null ? '—' : `${n(d.return_rate)!.toFixed(2)}%`,
    },
    { label: 'Profit / Loss', value: money(d.pnl, 'HKD') },
    ...
    <div className="divide-y divide-border overflow-hidden rounded-card border border-border bg-surface">
      {rows.map((r) => (
        ...
        >
          ...
          <span className="text-[15px] text-text-primary">{r.value}</span>
        </div>
      ))}
    </div>
  )
