...
- Favor clear, concise docstring comments. Be sparing with comments outside docstrings. Keep comments up-to-date with code changes.
...
- **Shared UI only.** Reusable components live in `/src/components`; never duplicate UI. Global constants in `/src/constants`; pure helpers in `/src/lib`. Keep components and helpers small and focused on single responsibility.
- **Clarity and DRY, but never sacrifice runtime performance to reuse code.** Pure, minimal and functional code. If reuse would add meaningful overhead on a hot path, prefer a fast purpose-built implementation and note why.
- **Troubleshooting:** Always identify root cause BEFORE fixing.
- **Refactoring:** Preserve existing functionality, make incremental changes when possible, and update related documentation.
