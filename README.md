...
- **docs/**
  - `00_PRD.md` — what we're building and why; the multi-module Home hub; scope (Wellness, Net Worth, Shows, Books, Quotes, Medical, Travel).
...
The app is **multi-module behind a Home hub** (Wellness, Net Worth, Shows, Books, Quotes, Medical, Travel; more later) — see `00_PRD.md`.
Wellness (food, supplements, activity, dashboard, library, settings, auth, sync).
Net Worth (monthly asset snapshots in HKD across 7 asset types; manual CSV import; **funds** driven by
...
reached via the Home hub; shares only auth/profile and the app shell).
...
Literature (classical Chinese poems & prose with search, filter, read-aloud (粵/國 via the Web Speech API), and favourites (♥); the corpus is an immutable **static asset** (`public/literature/**`, generated by `npm run build:literature` from a gitignored `poems.db`) for fully-offline reading at zero DB cost, while favourites + read-aloud settings sync via Supabase; built fresh to WellWorth conventions — shares only auth/profile and the app shell).
