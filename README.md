...
Shows (TV, movies & documentaries, incl. Chinese titles; Chinese-aware TMDB metadata, favourites (♥), a Poster URL field that shows only when TMDB has no poster (or forced on in Settings) + per-show Refresh, dashboard shelves, a filterable/sortable library, field-visibility settings, and a CSV importer; shares only auth/profile and the app shell).
